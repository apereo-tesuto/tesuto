...
- Install Postgres. Create databases for services (see below for list based on installation type). Version 9.4.x or higher is recommended.
...
2. Make note of the absolute path on your machine to where the `UI_ROOT_TARGET` directory is within Tomcat
3. Edit the `UI_ROOT_SOURCE/environment.json` file in the source code and modify "target" to
   the absolute path mentioned in the step above
4. Now via the command line navigate to the 'UI_ROOT_SOURCE' in your source code where the 'Gruntfile.js' is located
...
Inline comments are supported in YAML, but not block comments. Any service in the
