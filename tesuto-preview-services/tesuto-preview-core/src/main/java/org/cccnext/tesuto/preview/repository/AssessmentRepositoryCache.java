...
@Service("assessmentCacheDao")
public class AssessmentRepositoryCache implements AssessmentRepository,InitializingBean {

	private Cache assessmentCache;
	@Value("${assessmentCacheName}")
	...
	public void afterPropertiesSet() {
		...
		assessmentCache = cacheManager.getCache(cacheName);
	}
    @Override
    public long count() {
        return 0;
	...
	public Iterable<Assessment> findAllById(Iterable<String> assessmentIds) {
		return this.findAllById(assessmentIds);
	}

	public Optional<Assessment> findById(String assessmentId) {
		return Optional.of(getOne(assessmentId));
	}
