...
public class AssignedPlacementMessageHandlerTest {
    ...
    @InjectMocks
    AssignedPlacementMessageHandler placementHandler = new AssignedPlacementMessageHandler();
    ...
    public void receiveWithRetryableMessageException() throws Exception {
        doThrow(new RetryableMessageException()).when(service).requestAssignedPlacement(any(PlacementActionResult.class));
    ...
    public void receiveWithException() throws Exception {
        doThrow(new RuntimeException()).when(service).requestAssignedPlacement(any(PlacementActionResult.class));
        MessageHandlingInstructions instructions = placementHandler.receive( testEvent);
        Assert.assertFalse(instructions.isDelete());
        Assert.assertFalse(instructions.isStoreMessage());
        Assert.assertTrue( instructions.getMinRetries() > 0);
    ...
    public void receiveWithoutException() throws Exception {
        doNothing().when(service).requestAssignedPlacement(any(PlacementActionResult.class));
        MessageHandlingInstructions instructions = placementHandler.receive( testEvent);
