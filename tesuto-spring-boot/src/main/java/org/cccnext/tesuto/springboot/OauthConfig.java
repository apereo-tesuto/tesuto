...
public class OauthConfig {
    ...
    @Bean(name="cryptoBearerFilter")
    CryptoBearerFilter cryptoBearerFilter() {
        CryptoBearerFilter bearer = new CryptoBearerFilter(jwkEndpoint);
        Map<String,List<String>> roleMap = new HashMap<>();
        roleMap.put("ROLE_API", Arrays.asList("API"));
        roleMap.put("ROLE_USER", Arrays.asList("USER"));
        bearer.setRoleMap(roleMap);
        return bearer;
    }
    ...
    ServiceAccountManager securityAccountManager() {
        ...
        return builder.baseEndpoint(baseEndpoint).clientId(clientId).clientSecret(clientSecret).scope("openid").scope("superuser").build();
    }
