...
public class PlacementFactServiceTest {
    
    @Autowired PlacementFactService service;
    ...
    private Map<String,Object> initFacts() {
        ...
        Integer subjectAreaVersion = randomInteger();
        return new HashMap<String,Object>() {{
            ...
            put(PlacementMapKeys.SUBJECT_AREA_VERSION_KEY, subjectAreaVersion.toString());
            put(PlacementMapKeys.EVENT_TRIGGER_KEY, PlacementMapKeys.EVENT_TRIGGER_PLACEMENT);
    ...
    public void testNoExecuted() {
        ...
        reader.addPlacementComponents((String) facts.get(PlacementMapKeys.MISCODE_KEY), 
                (String)facts.get(PlacementMapKeys.CCCID_KEY),
               Integer.parseInt( (String)facts.get(PlacementMapKeys.SUBJECT_AREA_KEY)), 
               Integer.parseInt( (String)facts.get(PlacementMapKeys.SUBJECT_AREA_VERSION_KEY)), 
               new String[]{PlacementMapKeys.MMAP_PLACEMENT_COMPONENT_CLASS_NAME,
                PlacementMapKeys.TESUTO_PLACEMENT_COMPONENT_CLASS_NAME}, 1);
    ...
    public void errorIfNoSourceDto() {
        Map<String, Object> facts = initFacts();
        reader.clearPlacementComponents();
        List<String> errors = service.processFacts(facts);
        assertEquals(1, errors.size());
    }
