...
validateYaml() {
  ruby -e "require 'yaml';puts YAML.load_file('/tmp/param.values')" > /dev/null
  if [ $? -ne 0 ]; then
...
COM_STRING="-Djava.security.egd=file:/dev/./urandom -Dspring.config.location=classpath:application.properties,/opt/ccc/config/drools-editor.properties -jar /app.jar"
