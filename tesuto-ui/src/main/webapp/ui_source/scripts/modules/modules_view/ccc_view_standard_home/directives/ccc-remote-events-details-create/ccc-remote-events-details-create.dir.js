...
(function () {
    ...
    angular.module('CCC.View.Home').directive('cccRemoteEventsDetailsCreate', function () {
        ...
        return {
            ...
            controller: [
                ...
                function ($scope, $element, $timeout, $filter) {
                    ...
                    $scope.remoteEvent.name = 'Remote Event ' + $filter('date')(new Date(), 'MM/dd/yy');
                    ...
                    var doSubmit = function () {
                        ...
                        if ($scope.remoteEventCreateForm.$dirty || $scope.remoteEvent.testEventId === false) {
                            ...
                            $scope.remoteEvent.save().then(function () {
                                ...
                                $timeout(function () {
                                    $scope.$emit('ccc-remote-events-details-create.saved', $scope.remoteEvent);
                                }, 200);
                    ...
                    $scope.cancel = function () {
                        ...
                        if ($scope.remoteEvent.testEventId === false) {
                            $scope.$emit('ccc-remote-events-details-create.cancel');
                        } else {
                            $scope.$emit('ccc-remote-events-details-create.done', $scope.remoteEvent);
                        }
                    };
                    ...
                    '<div class="row">',
                        '<div class="col-md-8 col-md-offset-2">',
                            ...
                            '<div class="ccc-form-submit-controls text-left">',
                                ...
                                '<button class="btn btn-primary btn-icon-right pull-right" ccc-focusable type="submit" ng-click="attemptDoSubmit()" ng-disabled="loading">',
                                    '<span translate="CCC_VIEW_HOME.CCC-REMOTE-EVENTS-ADD.BUTTON_SELECT_STUDENTS" ng-if="remoteEvent.testEventId !== false && !remoteEventCreateForm.$dirty"></span> ',
                                    '<span translate="CCC_VIEW_HOME.CCC-REMOTE-EVENTS-ADD.BUTTON_SAVE_AND_SELECT_STUDENTS" ng-if="remoteEvent.testEventId === false || remoteEventCreateForm.$dirty"></span> ',
                                    '<i class="fa fa-chevron-right noanim" aria-hidden="true" ng-if="!loading || !submitted"></i>',
                                ...
                                '<button class="btn btn-default btn-icon-left btn-full-width-when-small" ng-click="cancel()" ng-disabled="loading">',
                                    ...
                                    '<span translate="Back" ng-if="remoteEvent.testEventId !== false && !remoteEventCreateForm.$dirty"></span> ',
                                    '<span translate="Cancel" ng-if="remoteEvent.testEventId === false || remoteEventCreateForm.$dirty"></span> ',
                                '</button>',
                            '</div>',
