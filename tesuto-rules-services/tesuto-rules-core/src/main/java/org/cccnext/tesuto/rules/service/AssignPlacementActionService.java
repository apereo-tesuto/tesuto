...
@Service
public class AssignPlacementActionService extends AbstractRestActionService {

    public final static String ASSIGNED_PLACEMENT_ACTION_NAME = "ASSIGNED_PLACEMENT_ACTION";

    @Value("${ASSIGN_PLACEMENT_ACTION_TARGET_URL}")
    public String placementTargetUrl;

    private final Set<String> requiredParameters = new HashSet<String>(Arrays.asList("assignedPlacement"));

    @Override
    public String getName() {
        return ASSIGNED_PLACEMENT_ACTION_NAME;
    }
    ...
    public Object buildContentBody(RulesAction action, Map<String, Object> facts) {
       action.getActionParameters().put("action", "add");
        PlacementViewDto placement = (PlacementViewDto) action.getActionParameters().get("assignedPlacement");
        if (placement != null) {
            return placement;
        }
        PlacementViewDto failedPlacement = new PlacementViewDto();
        ...
        failedPlacement.setSubjectAreaName((String)facts.get(PlacementMapKeys.SUBJECT_AREA_KEY));
